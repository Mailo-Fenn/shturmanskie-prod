...
(function () {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function (parameters) {
			...
			this.lenguage = parameters.lenguage;
			...
			if (this.result.ORDER_PROP && this.result.ORDER_PROP.properties) {
				...
				for (i = 0; i < props.length; i++) {
					if (props[i].TYPE == 'FILE') {
						prop = this.orderBlockNode.querySelector('label[data-property-id-row="' + props[i].ID + '"]');
						if (prop)
		...
		showError: function (node, msg, border) {
			...
			var msgs = msg.split('Имя');
			msg = msgs.join('First Name');
			msgs = msg.split('Фамилия');
			msg = msgs.join('Last Name');
			msgs = msg.split('Номер телефона');
			msg = msgs.join('Phone Number');
			...
			if (BX.saleOrderAjax && this.result.ORDER_PROP && this.result.ORDER_PROP.properties) {
				...
				for (i = 0; i < this.result.ORDER_PROP.properties.length; i++) {
					...
					if (curProp.TYPE == 'LOCATION' && curProp.MULTIPLE == 'Y' && curProp.IS_LOCATION != 'Y') {
						...
						if (curProp.CODE == 'CITY') {
							attrObj.altLocationPropId = parseInt(curProp.ID);
							this.deliveryLocationInfo.city = curProp.VALUE[0]
						}
		...
		locationsCompletion: function () {
			...
			for (i in this.locations) {
				...
				locationNode = this.orderBlockNode.querySelector('label[data-property-id-row="' + i + '"]');
				if (!locationNode)
					continue;
				...
				if (clearButton) {
					BX.bind(clearButton, 'click', function (e) {
						var target = e.target || e.srcElement,
							parent = BX.findParent(target, { tagName: 'LABEL', className: 'form-group' }),
							locationInput;
		...
		clickOrderSaveAction: function (event) {
			...
			Array.from(document.querySelectorAll('div[id="bx-soa-total"] .last.checkbox input')).forEach(item => {
				if (!item.checked) {
					checkbox = false;
				}
			})

			if (this.isValidForm() && checkbox) {
				this.allowOrderSave();
		...
		createBasketItem: function (basketItemsNode, item, index, active) {
			...
			if (item.data.PREVIEW_PICTURE_SRC && item.data.PREVIEW_PICTURE_SRC.length) {
				var image = BX.create('DIV', {
					props: { className: 'checkout-summary__product-image' },
					children: [
						BX.create('IMG', {
							props: { src: item.data.PREVIEW_PICTURE_SRC },
						})
					]
				});
			}
				...
				if (item.data.DETAIL_PICTURE && parseInt(item.data.DETAIL_PICTURE) > 0) {					
							
					var image = BX.create('DIV', {
						props: { className: 'checkout-summary__product-image' },
						children: [
							BX.create('IMG', {
								props: { src: document.getElementById('preview_id_' + item.data.DETAIL_PICTURE).innerText },
							})
						]
					});				
			...
			var count = BX.create('DIV', {
				props: { className: 'checkout-summary__product-count' },
				children: [
					BX.create('SPAN', {
						html: this.lenguage == 'ru' ? 'Кол.во: ' : 'QTY: ',
					}),
					BX.create('INPUT', {
						props: {
							type: 'number',
			...
			var content = BX.create('DIV', {
				props: { className: 'checkout-summary__product-content' },
				children: [
					BX.create('H3', {
						props: { className: "checkout-summary__product-name" },
						html: this.lenguage == 'ru' ? item.data.NAME : item.data.PROPERTY_NAME_ENG_VALUE
					}),
					BX.create('P', {
						props: { className: "checkout-summary__product-price" },
						html: item.data.SUM
					}),
					count
				]
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: { className: 'checkout-summary__product' },
		...
		getDeliveryLocationInput: function (node) {
			...
			if (location && location[0] && location[0].output) {
				...
				labelHtml = '<span>'
					+ BX.util.htmlspecialchars(this.lenguage == 'ru' ? currentProperty.NAME : 'Country (in English)...')
			...
			if (altProperty) {
				...
				input = BX.create('INPUT', {
					props: {
						...
						autocomplete: 'city',
						className: 'bx-soa-customer-input bx-ios-fix',
		...
		getZipLocationInput: function (node) {
			...
			if (zipProperty) {
				...
				propsItemNode = BX.create('LABEL', { props: { className: "form-group bx-soa-location-input-container" } });
				propsItemNode.setAttribute('data-property-id-row', zipProperty.ID);

				labelTextHtml = BX.util.htmlspecialchars(this.lenguage == 'ru' ? zipProperty.NAME : 'Postal code');
		...
		editPaySystemItems: function (paySystemNode) {
			...
			var paySystemItemsContainer = BX.create('DIV', { props: { className: 'checkout-section__list' } }),
				paySystemItemNode, i;
		...
		createPaySystemItem: function (item) {
			...
			if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y') {
				title = BX.create('P', { text: item.NAME });
			}
		...
		editDeliveryItems: function (deliveryNode) {
			...
			var deliveryItemsContainer = BX.create('DIV', { props: { className: 'checkout-section__list' } }),
				deliveryItemNode, k;
		...
		editPropsItems: function (propsNode) {
			...
			var propsItemsContainer = BX.create('DIV', { props: { className: '' } }),
		...
		getPropertyRowNode: function (property, propsItemsContainer, disabled) {
			...
			if (property.getName() == 'Индекс') {
				propsItemNode = BX.create('LABEL', {
					props: { className: 'form-group' },
					events: {
						change: BX.delegate(function (event) {
							document.querySelector('input[id="zipProperty"]').value = event.target.value;
						}, this)
					}
				})
			}
			...
			if (disabled) {
				...
				if (property.getId() == 17) {
					textHtml += BX.util.htmlspecialchars(this.lenguage == 'ru' ? property.getName() : 'First name');
				}
				else if (property.getId() == 18) {
					textHtml += BX.util.htmlspecialchars(this.lenguage == 'ru' ? property.getName() : 'Last name');
				}
				else if (property.getId() == 2) {
					textHtml += BX.util.htmlspecialchars(this.lenguage == 'ru' ? property.getName() : 'Email');
				}
				else if (property.getId() == 3) {
					textHtml += BX.util.htmlspecialchars(this.lenguage == 'ru' ? property.getName() : 'Phone number');
				}
				else if (property.getId() == 5) {
					textHtml += BX.util.htmlspecialchars(this.lenguage == 'ru' ? property.getName() : 'City');
				}
				else if (property.getId() == 20) {
					textHtml += BX.util.htmlspecialchars(this.lenguage == 'ru' ? property.getName() : 'Country');
				}
				else if (property.getId() == 19) {
					textHtml += BX.util.htmlspecialchars(this.lenguage == 'ru' ? property.getName() : 'Adress 2 (optional)');
				}
				else if (property.getId() == 21) {
					textHtml += BX.util.htmlspecialchars(this.lenguage == 'ru' ? property.getName() : 'Street');
				}
				else if (property.getId() == 22) {
					textHtml += BX.util.htmlspecialchars(this.lenguage == 'ru' ? property.getName() : 'Home');
				}
		...
		insertDateProperty: function (property, propsItemNode, disabled) {
			...
			if (disabled) {
				prop = this.propsHiddenBlockNode.querySelector('label[data-property-id-row="' + property.getId() + '"]');
				if (prop) {
					values = [];
		...
		insertFileProperty: function (property, propsItemNode, disabled) {
			...
			if (disabled) {
				...
				saved = this.savedFilesBlockNode.querySelector('label[data-property-id-row="' + property.getId() + '"]');
				if (saved)
		...
		alterProperty: function (settings, propContainer) {
			...
			for (i = 0; i < inputs.length; i++) {
				...
				BX.addClass(inputs[i], 'bx-soa-customer-input bx-ios-fix');

				if (inputs[i].name == 'ORDER_PROP_5' && this.lenguage == 'en') {
					BX.bind(inputs[i], 'change', e => {
						this.sendRequest();
					})
				}
			}
			...
			if (add.length) {
				...
				BX.bind(add, 'click', BX.delegate(function (e) {
					...
					if (textInputs.length) {
						...
						BX.addClass(textInputs[textInputs.length - 1], 'bx-soa-customer-input bx-ios-fix');
		...
		showValidationResult: function (inputs, errors) {
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				formGroup = BX.findParent(input0, { tagName: 'LABEL', className: 'form-group' }),
			...
			for (i = 0; i < inputs.length; i++) {
				inputDiv = BX.findParent(inputs[i], { tagName: 'LABEL', className: 'form-group' });
		...
		validateLocation: function (input, arProperty, fieldName) {
			...
			var parent = BX.findParent(input, { tagName: 'LABEL', className: 'form-group' }),
		...
		editPropsComment: function (propsNode) {
			...
			label = BX.create('LABEL', {
				...
				props: { className: 'bx-soa-customer-label', style: 'display: none;' },
			...
			div = BX.create('DIV', {
				props: { className: 'form-group bx-soa-customer-field', style: 'display: none;' },
		...
		editTotalBlock: function () {
			...
			if (this.result.DELIVERY.length) {
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(this.lenguage == 'ru' ? 'Стоимость доставки' : 'Shipping cost', deliveryValue, params));
			}

			// Итого
			this.totalInfoBlockNode.appendChild(this.createTotalUnit(this.lenguage == 'ru' ? 'Итого' : 'Subtotal', priceHtml, params));
			...
			if (this.options.showPayedFromInnerBudget) {
				this.totalInfoBlockNode.appendChild(this.createTotalUnit("Общая сумма заказа", total.ORDER_TOTAL_PRICE_FORMATED));
				...
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(this.lenguage == 'ru' ? "Общая сумма заказа" : "Order total", total.ORDER_TOTAL_PRICE_FORMATED, { total: true }));
			}
			...
			this.totalInfoBlockNode.appendChild(
				BX.create('LABEL', {
					...
					children: [
						...
						BX.create('DIV', {
							html: this.lenguage == 'ru' ?
								'Вводя свои персональные данные вы соглашаетесь с условиями использования' :
								'By entering your personal information you agree with the terms of use',
						}),
					],
					// events: {
					// 	click: BX.proxy(this.clickOrderSaveAction, this)
				...
				BX.create('LABEL', {
					...
					children: [
						...
						BX.create('DIV', {
							...
							html: this.lenguage == 'ru' ?
								`Установив этот флажок, вы соглашаетесь с тем, что прочитали и приняли Условия использования Sturmanskie и что ваши данные будут обрабатываться Sturmanskie в соответствии с ее Политикой конфиденциальности. Пожалуйста, ознакомьтесь с этой политикой для получения всех подробностей, касающихся обработки (включая использование, передачу и хранение) ваших данных, а также для получения дополнительной информации о ваших правах и возможностях в отношении такой обработки.` :
								`By checking this box, you agree that you have read and accepted the Sturmanskie terms of use and that your data will be processed by Sturmanskie, in conformity with its Privacy Policy. Please refer to this policy for all details surrounding the processing (including use, transfers and retention) of your data, as well as to learn more about your rights and choices regarding such processing.`,
						}),
					],
					// events: {
					// 	click: BX.proxy(this.clickOrderSaveAction, this)
		...
		createTotalUnit: function (name, value, params) {
			var totalValue, className = 'cart-summary__sum uppercase';
			...
			return BX.create('DIV', {
				...
				children: [
					BX.create('SPAN', { props: { className: 'cart-summary__sum-name' }, text: name }),
					BX.create('SPAN', {
						props: {
							className: 'cart-summary__sum-payment'
						},
						children: totalValue
					})
				]
