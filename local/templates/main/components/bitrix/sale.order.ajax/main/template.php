...
if ((string) $request->get('ORDER_ID') !== '') {
	...
	<link rel="stylesheet" href="/bitrix/components/bitrix/main.userconsent.request/templates/.default/style.css">
	<link rel="stylesheet" href="/bitrix/components/bitrix/sale.location.selector.steps/templates/.default/style.css">
	<link rel="stylesheet" href="/bitrix/components/bitrix/socserv.auth.form/templates/.default/style.css">
	<link rel="stylesheet" href="<?= '/bitrix/css/main/themes/' . $arParams['TEMPLATE_THEME'] . '/style.css' ?>">
	<link rel="stylesheet" href="<?= $templateFolder . '/style.css' ?>">
	<?
	$bLen = count($arResult['BASKET_ITEMS']);

	for ($i = 0; $i < $bLen; $i++) {
		$photo = CFile::ResizeImageGet((int) $arResult['BASKET_ITEMS'][$i]['DETAIL_PICTURE'], array('width' => 210, 'height' => 210), BX_RESIZE_IMAGE_PROPORTIONAL, true);
		echo '<div id="preview_id_' . $arResult['BASKET_ITEMS'][$i]['DETAIL_PICTURE'] . '" style="display: none;">' . $photo['src'] . '</div>';
	}
	?>
	<div class="container zero-padding first">
		<form action="<?= POST_FORM_ACTION_URI ?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form"
			...
			<div id="bx-soa-order" class="checkout-grid">
				<!--	MAIN BLOCK	-->
				<div class="checkout-form form">
					<h2 class="section-title"><?= $LENGUAGE == 'ru' ? 'Оформить заказ' : 'Checkout' ?></h2>
					...
					<div id="bx-soa-properties" data-visited="false" class="bx-soa-section bx-active">
						<h3 class="margened-title"><?= $LENGUAGE == 'ru' ? 'Адрес доставки' : 'Shipping adress' ?></h3>
					...
					<div id="bx-soa-delivery" data-visited="false"
						class="bx-soa-section bx-active checkout-delivery checkout-section">
						<h2 class="section-title"><?= $arParams['MESS_DELIVERY_BLOCK_NAME'] ?></h2>
					...
					<div id="bx-soa-paysystem" data-visited="false"
						class="bx-soa-section bx-active checkout-delivery checkout-section">
						<h2 class="section-title"><?= $arParams['MESS_PAYMENT_BLOCK_NAME'] ?></h2>
				...
				<div class="checkout-summary">
					<div class="checkout-summary__wrapper">
						<h2 class="section-title"><?= $LENGUAGE == 'ru' ? 'Итого' : 'Order summary' ?></h2>

						<div id="bx-soa-basket" data-visited="false"
							class="bx-soa-section bx-active checkout-summary__list">
							<div class="bx-soa-section-content container-fluid checkout-summary__list"></div>
						</div>
