...
<?php
    ...
    if (CModule::IncludeModule("sale")) {
        ...
        while ($arBasketItem = $dbBasketItems->Fetch()) {
            if ($arBasketItem["PRODUCT_ID"] == $arResult["ID"]) {
                $isInCart = true;
    ...
    <div class="container first">
        <div class="bread center">
            <?php
                ...
                $res = CIBlockElement::GetElementGroups($arResult['ID'], true , $arSelect2);
                while($ob = $res->Fetch()){
                    if($ob['DEPTH_LEVEL'] == 1){
                        $parentSection = $ob;
                    }
            ...
            <div class="bread__wrapper">
                ...
                <?php } ?>
                <?=$LENGUAGE == 'ru' ? $arResult['NAME'] : $arProps['NAME_ENG']['VALUE'] ?>
            </div> 
        </div>
        ...
        <div class="product-info__wrapper">
            ...
            <div class="product-info__content">
                ...
                <h1 class="product-info__title left-margin"><?=$LENGUAGE == 'ru' ? $arResult['NAME'] : $arProps['NAME_ENG']['VALUE'] ?></h1>

                <div class="product-info__cost left-margin">
                    <?php
                        $productId = $arResult['ID'];
                        $catalogGroupID = SITE_ID == 's2' ? 2 : 1;
                        $prices = array(
                            'price' => $arResult['ITEM_PRICES'][0]['UNROUND_PRICE'],
                            'base_price' => $arResult['ITEM_PRICES'][0]['UNROUND_BASE_PRICE'],
                        );
                        ...
                        if($LENGUAGE == 'ru'){
                            ...
                            if($prices['price'] != $prices['base_price']){
                                echo ' <span class="product-info__cost-old">' . $prices['base_price'] . ' ₽</span>';
                            }
                        } else {
                            echo "€ ".$prices['price'];
                ...
                <?php
                    ...
                    $dbSections = CIBlockElement::GetElementGroups($arResult['ID'], true, array());
                    ...
                    /*if ($subsection_id) {
                        echo "<ul class='product-info__variants'>";
                            ...
                            $arFilter = array(
                                "IBLOCK_ID" => 2, // Укажите нужный ID инфоблока
                            ...
                            while ($ob = $res->GetNextElement()) {
                                $arItem = $ob->GetFields();
                ?>
                                <li class="<?=$arItem['ID'] == $arResult['ID'] ? 'active' : '' ?>">
                ...
                <form method="post" action="<?=POST_FORM_ACTION_URI?>" class="add-to-cart-form">
                    ...
                    <input type="hidden" name="TYPE_PRICE" value="EUR">
...
<?php
if(
    ($LENGUAGE == 'ru' && isset($arResult['~DETAIL_TEXT']) && $arResult['~DETAIL_TEXT'] != "") || 
    ($LENGUAGE != 'ru' && isset($arProps['DESC_ENG']['~VALUE']['TEXT']) && $arProps['DESC_ENG']['~VALUE']['TEXT'] != "" )
){
?>
<section class="product-description dark">
    <div class="container hd-size top-ultra bottom-min">
        <div class="product-description__wrapper">
            ...
            <?=$LENGUAGE == 'ru' ? $arResult['~DETAIL_TEXT'] : $arProps['DESC_ENG']['~VALUE']['TEXT'] ?>
        </div>
    </div>
</section>
<?php } ?>
